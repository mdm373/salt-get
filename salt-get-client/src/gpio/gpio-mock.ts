...
const pinStreams: {[key: number]: WatchData[]} = {
  ...
  14: [{level: 1, tick: 3356606804, delta: undefined}, {level: 0, tick: 3356614079, delta: 7275}],
  15: [{level: 1, tick: 3356606804, delta: undefined}, {level: 0, tick: 3356614079, delta: 7275}],
}
...
export const getMockPin: PinProvider = (num, dir, edge) => {
  ...
  const watch = () => {
    ...
    if (pinStreams[num]) {
      ...
      const clear = setInterval(() => copy[0] ? subj.next(copy.pop()) : clearInterval(clear), 1)
    }
